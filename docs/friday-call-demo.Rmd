...
source_credentials <- scrape_user_api_tokens(source_conn, my_username)
...
We will generate some records for your main project for you to port over. First, get your credentials to the REDCap project you want to read from.
...
for (record_id in 1:rows_to_simulate) {
...
for (tube in 1:30) {
  simulated_data <- simulated_data %>%
    mutate(
      ...
      "tube_volume{tube}" := sample(1:30, size = 1)
    )
}
